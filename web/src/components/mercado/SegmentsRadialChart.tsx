...
export default function SegmentsRadialChart() {
  ...
  const { chartData, chartConfig } = useMemo(() => {
    ...
    const top = [...segments].sort((a, b) => b.value - a.value).slice(0, TOP_N)
  ...
  const formatValue = (value: number) =>
    value >= 1000 ? `R$ ${(value / 1000).toFixed(2)}bi` : `R$ ${value}MM`
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Maiores faturamentos no {quarterOrdinalLabel(quarter)} de {year}
          </p>
        </div>
